...
max_train_samples = params.nSamples; %max stored samples.
...
% construct cosine hanning window
...
% http://www.p-chao.com/2017-02-17/%E5%9B%BE%E5%83%8F%E8%B7%9F%E8%B8%AA%EF%BC%88%E4%BA%94%EF%BC%89srdcf%E7%9A%84%E6%95%B0%E5%AD%A6%E6%8E%A8%E5%AF%BC/
reg_window_edge = {};
...
%DSST http://blog.csdn.net/gxb0505/article/details/52601613?locationNum=8
...
for frame = 1:num_frames
    ...
    tic();
    rect_position_gt = params.ground_truth(frame,:);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if frame > 1
        ...
        iter = 1;
        location_params = {params.newton_iterations,img_support_sz,output_sz,min_scale_factor,max_scale_factor,base_target_sz,scaleFactors};
        %translation search
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            %full_fourier_coeff is hf; Convolution operator to get
            ...
            scores_fs_1 = permute(scores_fs_feat{1}, [1 2 4 3]);
            scores_fs_2 = permute(scores_fs_feat{2}, [1 2 4 3]);
            scores_fs_avg = permute(sum(cell2mat(scores_fs_feat), 3), [1 2 4 3]);
            location_1 = get_location(scores_fs_1,location_params,currentScaleFactor,sample_pos);
            location_2 = get_location(scores_fs_2,location_params,currentScaleFactor,sample_pos);
            location_avg = get_location(scores_fs_avg,location_params,currentScaleFactor,sample_pos);
            crop_img1 = imcrop(im_to_show,location_1);
            crop_img2 = imcrop(im_to_show,location_2);
            [w1,w2] = iou(location_1,location_2,params.ground_truth(frame,:));
            %[w1,w2] = correlation(crop_img1,crop_img2,pre_crop_img);
            %[w1,w2] = SSIM(crop_img1,crop_img2,pre_crop_img);
            %[w1,w2] = PSNR(crop_img1,crop_img2,pre_crop_img);
            %[w1,w2] = SIFT(crop_img1,crop_img2,pre_crop_img);
            disp([w1,w2]);
            scores_fs_feat{1} = scores_fs_feat{1} * w1;
            scores_fs_feat{2} = scores_fs_feat{2} * w2;           
            %scores_fs = permute(sum(cell2mat(scores_fs_feat{1}), 3), [1 2 4 3]);
            scores_fs = permute(scores_fs_feat{1}, [1 2 4 3]);                        
            ...
            imagesc(im_to_show);
            hold on;
            %plot(location_1(1),location_1(2),'r*');
            rectangle('Position',location_1, 'EdgeColor','b', 'LineWidth',2);
            rectangle('Position',location_2, 'EdgeColor','y', 'LineWidth',2);
            rectangle('Position',rect_position_vis, 'EdgeColor','w', 'LineWidth',2); 
            rectangle('Position',location_avg, 'EdgeColor','g', 'LineWidth',2);
            rectangle('Position',rect_position_gt, 'EdgeColor','r', 'LineWidth',2);  
            %pre_crop_img = imcrop(im_to_show,rect_position_vis);
    ...
    elseif params.learning_rate > 0 
        ...
        % Shift the sample so that the target is centered %shift previous
        % traget window so that the target is centered.
        shift_samp = 2*pi * (pos - sample_pos) ./ (sample_scale * img_support_sz);
    ...
    %use distances to select samples for training.
    ...
    %https://zhuanlan.zhihu.com/p/24971525
