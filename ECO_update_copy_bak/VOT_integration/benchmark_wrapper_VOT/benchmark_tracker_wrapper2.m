...
%diary('/local/home/share/fanyingruo/work2017/vot2016_new/vot-toolkit-vot2016-final/EVELYN_TRAX/log.txt');
%diary on;
%disp('lll')
...
cnn_params.output_layer = [3 10];               % Which layers to use
...
while true
    ...
    if frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            scores_fs_feat{1}=scores_fs_feat{1};
            scores_fs_feat{2}=scores_fs_feat{2};
	    %scores_fs_feat{3}=scores_fs_feat{3};
            %scores_fs_feat{1,1} = 1 *scores_fs_feat{1,1};
            %scores_fs_feat{1,2} = 2 *scores_fs_feat{1,2};
