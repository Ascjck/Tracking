...
%run('/fast/Tracking/ECO_update_copy_bak/external_libs/vlfeat-0.9.21/toolbox/vl_setup')
...
% **********************************
[handle,image,region] = vot_new('polygon'); %call vot_new.m function   
...
%get GT list
seq_id = strfind(seq.s_frames,'/');
seq_dir = seq.s_frames(1:seq_id(7));
seq_gt = [seq_dir,'groundtruth.txt'];
ground_truth = dlmread(seq_gt);
...
%cnn_params.compressed_dim = [3 64 256];            % Compressed dimensionality of each output layer
cnn_params.input_size_mode = 'adaptive';        % How to choose the sample size
cnn_params.m = 1;                % Extra scale factor of the input samples to the network (1 is no scaling)
...
while true
    ...
    if frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            %try
            %disp("ok1");
            num_point = size(ground_truth(frame,:))
            num_point = num_point(2);
            %disp(num_point);
	    x_location = []; y_location = [];
            if num_point == 4
            	gt_box = [ground_truth(frame,1),ground_truth(frame,2),ground_truth(frame,3),ground_truth(frame,4)];
	    else
		    x_count = 1; y_count=1;
		    for i = 1:num_point
		    	if mod(i,2) == 0
				y_location(y_count) = ground_truth(frame,i);
				y_count = y_count + 1;
			else
				x_location(x_count) = ground_truth(frame,i);
				x_count = x_count + 1;
			end
		    end
		    %disp("ok2");
                    %disp(x_location);
	   	    %disp(y_location);
		    %x_location = [ground_truth(frame,1),ground_truth(frame,3),ground_truth(frame,5),ground_truth(frame,7)];
		    %y_location = [ground_truth(frame,2),ground_truth(frame,4),ground_truth(frame,6),ground_truth(frame,8)];
		    upper_left_x = min(x_location); upper_left_y = min(y_location);
		    bottom_right_x = max(x_location); bottom_right_y = max(y_location);
		    w = bottom_right_x - upper_left_x;
		    h = bottom_right_y - upper_left_y;
		    gt_box = [upper_left_x,upper_left_y,w,h];
            end
	    %catch err
	    ...
	    %imagesc(im);   
	    %hold on;
	    %rectangle('Position',gt_box, 'EdgeColor','y', 'LineWidth',2);
            %gfframe=getframe(gcf);
            %save_path = ['/fast/Tracking/temp/', num2str(frame),'.jpg'];
	    %imwrite(gfframe.cdata,save_path);
	    %disp("ok3");
            ...
            %crop_img1 = crop(im_to_show,location_1);
            %crop_img2 = crop(im_to_show,location_2);
            [w1,w2] = iou(location_1,location_2,gt_box);
	    %disp([w1,w2]);
	...
	rect_position(frame,:) = round([pos([2,1]) - (target_sz([2,1]) - 1)/2, target_sz([2,1])]);
        ...
        im_to_show = double(im)/255;
        pre_crop_img = imcrop(im_to_show,rect_position_vis);
	%record the first frame GT for comparsion
